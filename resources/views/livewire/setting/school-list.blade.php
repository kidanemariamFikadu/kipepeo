<div>
    ...
    <div class="p-2 md:p-6">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-2xl font-semibold text-gray-700 dark:text-white">Schools</h2>
    ...
    <div class="relative bg-white rounded-lg shadow dark:bg-gray-800 p-4">
        ...
        @if ($this->schoolList->isEmpty())
            <p class="px-4 pb-4 text-sm text-gray-500 dark:text-gray-400">No schools found.</p>
        @else
                ...
                @foreach ($this->schoolList as $school)
                    <div class="flex items-center justify-between gap-3 py-2 px-4 border-t dark:border-gray-700">
                        ...
                        <div class="flex items-center gap-1 shrink-0">
                            <a href="{{ route('school-detail', $school->id) }}" title="View details"
                            ...
                            <button title="Remove school"
                                wire:confirm="Delete {{ $school->name }}? This can't be undone from here."
                                wire:loading.attr="disabled" wire:target="removeSchool({{ $school->id }})"
                                ...
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                    wire:loading.remove wire:target="removeSchool({{ $school->id }})">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                </svg>
                                <x-spinner class="h-4 w-4" wire:loading wire:target="removeSchool({{ $school->id }})" />
                            </button>
                        </div>
                    </div>
                @endforeach
